...
for theseeds in removedSeeds {
	...
	if ("gold".matches(theseeds) || "diamond".matches(theseeds) ||  "blaze".matches(theseeds)) {
		print("不再在JEI中隐藏 "~theseeds~" 种子，因为现在可以通过任务获得。");
	} else {
		...
		itemUtils.getItem("mysticalagriculture:"~theseeds~"_seeds").addTooltip(format.red("你处于Normal Packmode，因此此物品被禁用且无用。"));
		itemUtils.getItem("mysticalagriculture:"~theseeds~"_essence").addTooltip(format.red("你处于Normal Packmode，因此此物品被禁用且无用。"));
	}
}
